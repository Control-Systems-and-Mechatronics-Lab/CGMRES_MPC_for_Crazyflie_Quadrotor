 ...
 * Solution structure
 */
 #define max_krylov_dim 4 // 최대 Krylov 차원

typedef struct {
  Matrix* Ac; // continuous Ac
  Matrix* Bc; // continuous Bc
  Matrix* AcT;
  Matrix* BcT;

  Matrix* lambda;  ///< Costate (Adjoint Variable)
  Matrix* eu;  //compute Optimality eu
  Matrix* eu_1;  //compute optimality eu1
  Matrix* eu_2b;  //compute optimality eu2 (bfunc)
  Matrix* eu_2ax;  // compute optimality eu2 (axfunc)

  Matrix* b;
  Matrix* b_zeta_0;
  Matrix* ax;

  Matrix* solution_update_vec;
  Matrix* current_solution_vec;
  Matrix* current_solution_vec_b;
  Matrix* current_solution_vec_ax;
  Matrix* solution_vec;
  ...
  Matrix* X_perturb; // compute phix, Hx, Hu

  Matrix* optix;
  Matrix* optiu;

  // Matrix* U_predict;
  // Matrix* U_b; // compute Hu
  // Matrix* U_ax;
  // Matrix* direc_vec;


  Matrix* basis_mat;
  Matrix* hessenberg_mat;
  //float(*givens_c)[max_krylov_dim + 1];
  //float(*givens_s)[max_krylov_dim + 1];
  //float(*g_vec)[max_krylov_dim + 1];
  Matrix* givens_c;
  Matrix* givens_s;
  Matrix* g_vec;

  
  // Matrix* phix;     ///< Final cost gradient
  Matrix* hx;       ///< hx = Q(x_k - x_ref) + A^T λ_{k+1}
  Matrix* hu;       ///< hu = R u_k + B^T λ_k
